...
conn = ibm_db.connect("DATABASE=bludb;HOSTNAME=125f9f61-9715-46f9-9399-c8177b21803b.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=30426;SECURITY=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID=ldm84139;PWD=FAgl73hor4nyJk6r;", "", "")
@app.route('/')    
def root():
...
app.config['MAIL_USERNAME'] = 'mailsending@gmail.com'
...
def requested():
    ...
    print(address)
    sql = "SELECT * FROM users WHERE blood=?"
    stmt = ibm_db.prepare(conn, sql)
    ibm_db.bind_param(stmt,1,bloodgrp)
    ibm_db.execute(stmt)
    data = ibm_db.fetch_assoc(stmt)
    return render_template('stats.html', pred="Your request is sent to the concerned people.")
    data = ibm_db.fetch_assoc(stmt)
    msg = Message('Hello',sender =[data.senderemail ],recipients = [data.receiveremail])
    msg.body = 'There has been a request for your blood'
    print ("Recipient Contact is : ", data["PHONE"])
    return render_template('stats.html', pred="Your request is sent to the concerned people.")
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=30875)
